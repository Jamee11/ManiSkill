...
- Result: Push and Pick each contain train/val/test `500/100/200` episodes. Across both tasks there are `1600` Parquet episodes, `3200` H.264 front/wrist videos and `111462` action-aligned frames. Every summary reports 44D RSIG supervision, front+wrist only, robot-base metric actions, no future-object targets and no goal/phase/memory policy input. The joint loader returns state `(1,60)`, action `(8,7)` and five temporal images `(3,224,448)`.
- Main-gradient preflight: added opt-in, ZeRO-2-safe diagnostics without changing the forward graph or total loss. Nonzero `action_role_projection`/`action_interaction_projection` gradients will isolate the Action main-loss path; nonzero `video_alpha` gradient will isolate the future-video main-loss path. At zero initialization, `video_out` is expected to receive no Video gradient on the first step and begin receiving it after the gate moves. Plan stop-gradient remains a focused unit-test assertion because `.grad` presence after DeepSpeed backward cannot diagnose it reliably; its direction/moving predictor is trained only by auxiliary supervision.
- Static verification: Python compilation, seven focused RSIG tests, shell syntax/dry-run and a standalone `safe_get_full_grad` numerical check pass. This is not a substitute for the real 2B backward.
- Blocker/next gate: GPU free memory was only `13.1-24.5 GiB` while all cards were at `89-100%` utilization. Historical matched batch-1 ZeRO-2 training used about `48.44-48.49 GiB/card`; the 7.65M-parameter RSIG increment is small, but four-GPU execution still requires at least about `52 GiB/card`, while true single-GPU execution is estimated near `70-71 GiB`. No training was launched into unrelated allocations. When sufficient memory is free, run the revised one-step smoke; after its path-specific gradients and live hidden shape pass, run the new four-GPU 20-step calibration launcher and inspect every raw loss, gate trajectory, peak memory and save/resume before 40k. Current logs are rank-0 final-microbatch values, suitable for finite/order-of-magnitude calibration but not a global effective-batch mean; diagnostic all-reduces also make this run unsuitable for throughput claims.
...
# 2026-07-20 UTC - Sim2Real frame、三视角与 RLBench GT-area 路线复核

- 范围：只读核查当前 ManiSkill EORT 环境/deriver/collector、DiT4DiT ManiSkill exporter/tracker bridge，以及 `push_button_single_eval`、`push_buttons_difficulty_eval`、`push_button_incremental_robustness`；未改采集、模型或训练代码，未启动新实验。
- Frame 证据：当前 sidecar 的 TCP/object/goal、segdepth centroid 和 learned-tracker bridge 使用 world frame；Panda root pose 为 `[-0.615,0,0]` 且 rotation identity。`pd_ee_delta_pose` controller 源码明确使用 root translation/root-aligned rotation，所以现有 metric command 语义本来就是 robot-root/base axes；由于 identity rotation，已有数据的相对向量数值与 base-frame 相同。结论是下一版应显式统一为 robot-base contract，但这本身不会改变当前样本数值或自动提高成功率；主要收益是消除真机接口歧义并支持非 identity base。
- Front calibration 证据：用户指定的 K 为 `[[646.140,0,641.678],[0,645.812,356.128],[0,0,1]]`，distortion 为 `[-0.0530,0.0581,0.00070,0.00077,-0.0170]`，cam2base translation 约 `[1.0003,0.0009,0.3798]m`，rotation 为 proper rotation，reprojection error 文件为 `0.5313`。若采用 1280×720 center-crop 720×720 再 resize 256×256，理想 pinhole K 约为 `[[229.739,0,128.596],[0,229.622,126.623],[0,0,1]]`。该 crop/undistort 约定尚未由真实图像验证，不能直接写死。
- 三视角现状：ManiSkill EORT task 当前只把 `base_camera` 提到 256；`panda_wristcam` 已存在但默认 hand camera 为 128；没有 EORT right-shoulder sensor。当前 sidecar、preview、LeRobot exporter 和 tracker 都只接受单 camera；LeRobot 只导 RGB MP4，三路 uint16 depth 应继续保存在 raw HDF5/无损 sidecar，不能压成普通 H.264。DiT4DiT 已有 RLBench front+wrist 多 video-key 路径，但 ManiSkill config 仍单视角，第三视角必须作为显式 ablation 而不是假定已被模型消费。
- 遮挡决策：不再把人工静态 occluder 放入下一版 production train；保留既有 env/class 和旧数据用于回归或 held-out 压力测试，不删除。visibility head 仍需要负样本，优先来自 wrist/right-shoulder 的自然出视野、机器人自遮挡和无效 depth，并在 train/val 分别报告每 camera × role 的 visible/valid 比例。
- RLBench single-button：baseline 12/12，GT-area 12/12，无差异且每 variation 仅 2 trials，属于 ceiling effect。
- RLBench difficulty：两个 `difficulty_summary.json` 都是 0/0，不能作为结果。24 个原始日志显示 baseline 与 GT-area 都是 1-button 4/4、2-button 0/4、3-button 0/4；每个 variation 仅 1 trial，且写 summary 时因 NumPy `int64` 不可 JSON 序列化而报错。因此该目录不能支持“GT-area 提升”的结论。
- RLBench incremental robustness：在 `push_buttons` 上，color aggregate 从 baseline 19/90=21.1% 提升到 GT-area 69/90=76.7%（+55.6pp）；nominal robustness 从 9/30=30.0% 到 23/30=76.7%（+46.7pp）；gray-table 从 8/30=26.7% 到 24/30=80.0%（+53.3pp）。在 `push_button` 上提升较小：87/90→90/90、28/30→30/30、26/30→30/30。探索性独立 Fisher 检验对三组 multi-button 对比分别约 `6.1e-14/6.4e-4/7.3e-5`；trial 共享 variation 且不一定独立，p 值只作量级参考。
- 结论边界：13D GT-area 含 perfect object center、ee-relative vector、fixed normal/radius、simulator progress 和 interaction state。它证明“结构化 oracle condition 对复杂多目标任务很有价值”，尤其减轻视觉中选择目标/阶段的困难；它不证明 learned tracker 能从 RGB-D 恢复这些信息，也不能区分增益来自 geometry、progress 还是 interaction-state shortcut。
- 训练路线：仍采用两阶段。先用 simulator segmentation/centroid/visibility/depth supervision 单独训练 tracker，held-out camera/domain 验证后冻结；再用冻结 tracker 在 policy train/val/test 上离线生成 deployable condition，并训练 DiT4DiT。不要现在新增一套纯 action-loss 联合 perception module。只有 learned-condition policy 已跑通、oracle/deployable-GT 明显优于 robot-only、且 tracker error 被证明是瓶颈后，才做小学习率联合 fine-tune；联合阶段必须保留 heatmap/visibility supervised loss，不能只靠 action loss。
- 下一版最小数据方案：新增而非覆盖 v3 Sim2Real env/schema；raw 同步保存 front/right-shoulder/wrist 三路 RGB-D、每帧 intrinsics/extrinsics、robot-base pose 和原 world GT；policy canonical view 全部派生为 robot-base frame。train 使用以真实标定为中心、episode 内固定的小幅 camera/hand-eye perturbation，加有限 table-color 与 lighting randomization；val/test 分开 nominal 和 calibrated perturbation。旧 fixed/camera-rand/occluded v2 保持可复现。
- 最大不确定性：当前 DiT4DiT calibration manifest 的 camera 命名与用户新指定 front 标定相冲突；right-shoulder 最终标定身份、wrist intrinsics/hand-eye、真实流的 undistort/crop/resize 和时间同步尚未闭合。若这一层错误，base-frame 公式、三视角和 tracker 网络即使都正确，也会产生系统性 3D 偏差，是当前最优先的 smoke gate。
- 后续只读标定核查：`RobotWorkspace` 的 `camera.launch.py` 将 `344422300343` 注册为 `/pc_arm/body_camera`、将 `339222070579` 注册为 `/pc_arm/wrist_camera`；`pre_pipe.py/infer.py` 将 `results_6_17_344522302193/cam2base_4x4.npy` 与 `/pc_arm/front_camera` 路径配套使用。实际同一时刻图像显示 `image` 为工作台右侧后方俯视、`front_image` 为机器人正对工作台、`wrist_image` 为夹爪俯视。因此应将 `344422300343` 认定为 right-shoulder/body camera，将 `344522302193` 认定为 front；现有 DiT4DiT calibration manifest 的两者命名相反，后续 v3 实现必须纠正。仍未找到 `339222070579` 的真机 intrinsics 或 camera-to-EE/hand-eye 文件，wrist 只能先提供显式配置接口并在缺失时 fail closed，不能伪称 real-matched。

## 2026-07-20 14:34:08 UTC — v3 真实端到端 smoke 与自然 visibility

- 设置：GPU1、`PushCubeEORTSim2Real-v1`、seed 1、CPU PhysX + GPU renderer、三相机 256×256、无人工挡板；独立输出 `/remote-home/jinminghao/datasets/maniskill_eort_v3_smoke_fixed_20260720`。先 native motion planning，再官方 replay 到 `pd_ee_delta_pose`，之后 v3 sidecar、三视角 preview、oracle LeRobot 和 DiT4DiT loader。
- 结果：native 1/1 success、controller replay 1/1 success，`T=72`。每相机 RGB `(73,256,256,3)`、depth/segmentation `(73,256,256,1)`。phase 为 approach/contact/moving/success=`40/0/15/17`。front 和 right shoulder 的 object/goal 都是 72/72 visible+segdepth-valid；wrist object 72/72，goal 33/72，说明自然视野变化提供 39 个 goal-negative 帧。
